...
def parse_args(img_url):
    ...
    parser.add_argument(
        ...
        type=str,
        default=r'C:\Users\86195\Desktop\biaopabn\output\ppyolov2_r50vd_dcn\best_model')
    parser.add_argument(
        '--seg_model_dir',
        dest='seg_model_dir',
        help='The directory of the segmentation model',
        type=str,
        default=r'C:\Users\86195\Desktop\biaopabn\output\deeplabv3p_r50vd\best_model')
    parser.add_argument(
        '--image_dir',
        dest='image_dir',
...
class MeterReader:
    ...
    def roi_crop(self, img, det_results):
        ...
        for res in det_results:
            ...
            cv2.imshow('{}'.format(str(res)), sub_img)
            cv2.waitKey(1)
    ...
    def erode(self, seg_results, erode_kernel):
        ...
        for i in range(len(seg_results)):
            temp = seg_results[i]['label_map']
            temp = temp.astype(np.uint8)
            eroded_results[i]['label_map'] = cv2.erode(
                temp, kernel)
        # cv2.imshow('分割模型预测结果', eroded_results)
        # cv2.waitKey(1)
    ...
    def predict(self,
        ...
        # print(seg_results[0]["label_map"])
        # seg_results = seg_results[0]["label_map"]
        # print(seg_results.shape)
        # seg_results = seg_results.astype(np.uint8)
        ...
        # cv2.imshow("res:", img)
        # cv2.waitKey(1)
        output = r'C:\Users\86195\Desktop\biaopabn\output\result\rrr.jpg'
...
def infer(args):
    ...
    if len(image_lists) > 0:
        ...
        for image in image_lists:
            res = meter_reader.predict(image, args.save_dir, args.use_erode,
...
if __name__ == '__main__':
    args = parse_args(r'C:\Users\86195\Desktop\biaopabn\20190822_16.jpg')
    infer(args)
